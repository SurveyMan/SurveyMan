...
function maxEntOutlierClassifier(s::Survey, data::Array, alpha::Float64)
    ...
    distrs = computeEmpiricalDistributions(computeFrequencyMap(s,data))
    ents = [H([distrs[q][(b[q][2]).id] for q in keys(b)]) for b in data]
    thresh = CIL(ents, samp -> sum(samp) / length(samp), alpha)
    qoMap -> (h = H([distrs[q][(qoMap[q][2]).id] for q in keys(qoMap)]) ; (h, h < thresh)), thresh
end

function maxLogLikelihoodClassifier(s::Survey, data::Array, alpha::Float64)
    distrs = computeEmpiricalDistributions(computeFrequencyMap(s,data))
    lls = [LL([distrs[q][d[q][2].id] for q in keys(d)]) for d in data]
    thresh = CIL(lls, samp -> sum(samp) / length(samp), alpha)
    qoMap -> (h = LL([distrs[q][(qoMap[q][2]).id] for q in keys(qoMap)]) ; (h, h < thresh)), thresh
end
